        ...
        if (summaryCache[chapterId]) {
            return new Response(
                JSON.stringify({
                    book,
                    chapter,
                    summary: summaryCache[chapterId].summary,
                    keywords: summaryCache[chapterId].keywords,
                    chapterId,
                }),
                { status: 200 }
            );
...
`;
        ...
        // Agora, gera palavras-chave representativas do capítulo
        const keywordPrompt = `
Liste de 5 a 10 palavras-chave representativas deste capítulo da Bíblia.
Separe por vírgula, apenas palavras, sem explicações:

${finalSummary}
`;
        const keywordResponse = await client.models.generateContent({
            model: "gemini-2.0-flash",
        ...
        summaryCache[chapterId] = { summary: finalSummary, keywords };
